...
**Версія 3X-UI: 3.5.0.** Посібник складено за цією версією і є актуальним для неї. Зведення змін 3.5.0 відносно 3.4.2 — у розділі [«Що нового в 3.5.0»](#що-нового-в-350).
...
- [Що нового в 3.5.0](#що-нового-в-350)
...
- [12. Вузли (мультипанель, master/slave)](#12-вузли-мультипанель-masterslave)
  ...
  - [12.9. Виправлення 3.5.0 (стабільність вузлів)](#129-виправлення-350-стабільність-вузлів)
  - [12.10. Вузли: нове в 3.3.0](#1210-вузли-нове-в-330)
...
Образ включає Fail2ban (за замовчуванням активний) для застосування лімітів IP за клієнтами. Fail2ban блокує порушників через `iptables`, що вимагає можливості `NET_ADMIN`. У `docker-compose.yml` вона вже надана через `cap_add`. З 3.5.0 у контейнері полагоджено **автопродовження сертифікатів**, випущених через SSL-меню: entrypoint запускає `crond` і заново реєструє cron-завдання acme.sh, а `docker-compose.yml` отримав окремий том для стану acme.sh — продовження переживає перестворення контейнера (раніше сертифікати мовчки закінчувалися через ~90 днів). При ручному запуску через `docker run` можливості потрібно додати самостійно, інакше блокування будуть лише логуватися, але не застосовуватися:
...
Повторне натискання перемикача відкриває вікно «Отключить двухфакторную аутентификацию» з підказкою «Введите код из приложения, чтобы отключить двухфакторную аутентификацию.». Після введення вірного коду (з 3.4.2 він перевіряється **на сервері**) прапорець і секрет очищаються, показується «Двухфакторная аутентификация была успешно удалена».
...
**Підтвердження кодом 2FA (з 3.4.2).** Якщо на панелі увімкнено двофакторну автентифікацію, зміна логіна/пароля додатково вимагає **ввести поточний код** із застосунку-автентифікатора. Відкривається вікно **«Зміна облікових даних»** з підказкою «Введите код из приложения, чтобы изменить учетные данные администратора.»; код (`twoFactorCode`) перевіряється на сервері, при невірному або порожньому коді зміна не застосовується. Те саме підтвердження тепер потрібне і при вимкненні 2FA (див. [2.2](#22-двофакторна-автентифікація-2fa--totp)).
...
| Использовать TLS (LDAPS) | «Использовать TLS (LDAPS)» (`useTls`) | **false** | За увімкнення використовується схема `ldaps://` (за замовчуванням — з перевіркою сертифіката сервера). |
| Пропускати перевірку TLS-сертифіката | «Пропускати перевірку TLS-сертифіката» (`ldapInsecureSkipVerify`) | **false** | Додано в 3.4.2. Вимикає перевірку сертифіката сервера при LDAPS — для внутрішніх/самопідписаних CA. Підказка: «Небезпечно — вимикає перевірку сертифіката сервера. Використовуйте лише з внутрішніми/недовіреними CA». Перемикач доступний **лише** при увімкненому «Использовать TLS (LDAPS)». |
...
У бічному меню панелі (і в мобільній шторці) з версії 3.4.2 є кнопка **«Документація»** (іконка книги) — відкриває офіційну документацію `https://docs.sanaei.dev/`. Також у 3.4.2 проведено наскрізне покращення **доступності**: значки-кнопки і клікабельні елементи отримали aria-підписи і активацію по Enter/Space (для скрінрідерів і навігації з клавіатури).
...
Тут `v25.6.8` — тег зі списку, який повертає `GET /getXrayVersion`. Версія обов'язково має бути присутня в цьому списку, інакше панель відповість відмовою. З версії 3.4.2 мінімально допустима для встановлення версія Xray піднята до **26.6.27**, а в комплекті 3.5.0 йде ядро **Xray 26.7.11**. Супутні зміни ядра: шифри Shadowsocks `none`/`plain` і VMess `none`/`zero` видалені (збережені конфіги переписуються автоматично: SS — на підтримуваний шифр, VMess — на `auto`), а незашифрований VLESS/Trojan-outbound на публічну адресу відхиляється при збереженні — ядро з таким конфігом не запустилося б.
...
Для **PostgreSQL** з 3.5.0 «Відновлення» приймає **три види файлів** (тип визначається за вмістом, а не за розширенням): архів `pg_dump` (`PGDMP`) — застосовується через `pg_restore --clean --if-exists --single-transaction …`; **SQLite-базу `.db`** (звичайний бекап) і **SQLite-міграційний `.dump`** — вони перевіряються, за потреби перезбираються та імпортуються в PostgreSQL **однією транзакцією** тим самим рушієм, що й `x-ui migrate-db --dsn` (при помилці PostgreSQL залишається недоторканим). Діалог вибору файлу приймає `.dump,.db` на обох СУБД; завантаження `pg_dump`-архіву на SQLite-панель дає зрозумілу помилку. Підказка прямо попереджає: «Це замінить усі поточні дані».
...
Кнопка «Завантажити файл міграції» (*Download Migration*) викликає `GET /getMigration`. З 3.5.0 вона показується **лише на PostgreSQL-панелях** і скачує `x-ui.db` — готову SQLite-базу, зібрану з даних PostgreSQL (шлях «назад на SQLite»). На SQLite-панелях кнопку прибрано як надлишкову: звичайний бекап `.db` тепер і так відновлюється прямо в PostgreSQL-панель.
...
Вибір, де фізично працює inbound: на локальній панелі чи на одному із зареєстрованих вузлів. З 3.5.0 список «Развернуть на» (як і фільтр «Вузли» над списком inbound) підтримує **пошук за введенням**, а над самим списком inbound з'явилося **поле пошуку** (за приміткою, портом і протоколом; працює разом із фільтром вузлів). Особливість реалізації: `nodeId = 0` нормалізується в `nil`, оскільки `0` — це не валідний id вузла, а артефакт прив'язки форми; `nil`/`0` означає локальну панель. При збереженні inbound на офлайн-вузлі можливий тост «изменение синхронизируется, когда узел переподключится». З версії 3.4.2 цей тост показується лише коли вузол справді офлайн або вимкнений (раніше він міг з'являтися і при збереженні на онлайн-вузол).
...
- **«Копировать ссылку»** (англ. *Copy URL*), **«Экспорт ссылок»**. З версії 3.4.2 сторінкова дія **«Експортувати всі посилання»** (`GET /panel/api/inbounds/allLinks`; вікно «Експорт усіх посилань inbound», файл «All-Inbounds») формує посилання через рушій підписок — застосовує шаблон ремарки до кожного клієнта і надає перевагу керованим Host-ендпоінтам (раніше використовувалася фіксована ремарка `inbound-email`).
...
Призначення: inbound WireGuard — VPN-тунель WireGuard, а не маскований проксі. Транспорт і TLS/REALITY до нього незастосовні.

**З версії 3.4.2 WireGuard переведено на мультиклієнтну модель** (як VLESS/VMess/Trojan/Shadowsocks/Hysteria). Сам inbound зберігає лише серверну частину; пристрої-піри тепер додаються як звичайні **клієнти** (див. розділ 8) — з автоматичною видачею адреси в тунелі, підтримкою підписок, лімітів трафіку/терміну та груп. Попередній інлайн-список «Peer'и / Додати peer» з форми inbound прибрано: новий inbound створюється без пірів, а кожному клієнту адреса видається автоматично.

Поля inbound (блок `settings`):

| Поле | Значення за замовчуванням | Опис |
|---|---|---|
| `secretKey` | — | Приватний ключ сервера (обов'язковий). Поруч кнопка генерації; публічний ключ (`Public Key`) виводиться з нього автоматично (лише для читання; окремо збережений `pubKey` прибрано) |
| `mtu` | `1420` | MTU інтерфейсу |
| `dns` | `1.1.1.1, 1.0.0.1` | **Нове в 3.4.2.** DNS, що записується в рядок `DNS =` клієнтського `.conf` |
| `noKernelTun` | `false` (вимкн.) | «TUN без kernel» — userspace-TUN замість ядерного |
| `domainStrategy` | (опціонально) | «Domain Strategy»: `ForceIP`, `ForceIPv4`, `ForceIPv4v6`, `ForceIPv6`, `ForceIPv6v4` |

**Параметри WireGuard у клієнта** — вкладка **«Облікові дані»** у вікні додавання/редагування клієнта (показуються, коли прив'язаний inbound — WireGuard):

| Поле | Опис |
|---|---|
| «Приватний ключ WireGuard» | Приватний ключ клієнта (редаговане, є кнопка «Перегенерувати»); при введенні з нього автоматично виводиться публічний ключ |
| «Публічний ключ WireGuard» | Лише для читання; обчислюється з приватного |
| «Спільний ключ WireGuard» (PSK) | Необов'язковий додатковий спільний ключ |
| «Дозволені IP WireGuard» | З 3.5.0 — **редаговане** (плейсхолдер `10.0.0.2/32`, підказка «Залиште порожнім для авто-видачі; записи розділяйте комами»). Порожньо — адреса видається автоматично; сервер перевіряє кожен запис (IP або CIDR), одиночні адреси нормалізує до `/32` і відхиляє адресу, зайняту іншим клієнтом цього inbound |

Адреса в тунелі видається сервером автоматично з підмережі inbound (за замовчуванням `10.0.0.0/24`: сервер займає `.1`, клієнти — починаючи з `.2`); якщо наявні клієнти використовують іншу `/24`, нові адреси видаються в тій самій підмережі.

#### Domain Strategy і вкладка Transport

Крім перелічених, у WireGuard-inbound є поле **Domain Strategy** (стратегія розв'язання доменів: `ForceIP`, `ForceIPv4`, `ForceIPv4v6`, `ForceIPv6`, `ForceIPv6v4`). Поле необов'язкове і записується у конфіг, лише якщо задане.
...
#### Дії inbound та експорт (з 3.5.0)

Меню рядка WireGuard-inbound тепер містить повний «мультиклієнтний» набір дій — **«Експортувати всі посилання»**, експорт за підпискою, **прив'язка/відв'язка клієнтів**, додавання до групи і видалення всіх клієнтів (раніше були лише експорт/скидання/клонування/видалення), а в списку відображається лічильник клієнтів. Для WireGuard вікно «Експорт усіх посилань» розділено на дві вкладки: **Config** (склеєні `.conf`-блоки, по одному на клієнта) і **Links** (персональні посилання `wireguard://`); «Копіювати» і «Завантажити» діють на відкриту вкладку. Для inbound, розміщеного на вузлі, `Endpoint` у клієнтському `.conf`/QR тепер вказує **адресу вузла**, а не головної панелі.

#### Конфігурація та шеринг клієнта WireGuard

У WireGuard-клієнта у вікнах «Інформація»/QR доступна **згортна картка конфігурації**: повний `.conf` (`[Interface]` з `PrivateKey`/`Address`/`DNS`/`MTU` і `[Peer]` з `PublicKey`/`PresharedKey`/`AllowedIPs = 0.0.0.0/0, ::/0`/`Endpoint`/`PersistentKeepalive`) з кнопками **Копіювати**, **Завантажити** і **QR**. Підтримується також посилання виду `wireguard://`/`wg://`, яке клієнт-застосунок розбирає в готовий `.conf`.
...
**З версії 3.5.0 MTProto — мультиклієнтний протокол** (рушій змінено на форк `mtg-multi`): один inbound обслуговує багато користувачів, кожен з яких — звичайний **клієнт** (див. розділ 8) зі своїм FakeTLS-секретом, квотою трафіку, терміном, перемикачем увімкнення, необов'язковим ad-tag і персональним посиланням `tg://proxy`. Попередню модель «один inbound = один секрет» скасовано: поле «Secret» на рівні inbound з форми прибрано, а при оновленні панелі наявний одиночний секрет автоматично перетворюється на першого клієнта цього inbound (ручних дій не потрібно).

Наслідки:

- Вкладка **«Транспорт» (Stream Settings) до цього inbound не застосовується**; клієнти ж тепер керуються як у решти протоколів — прив'язка/відв'язка, ліміти, групи, підписки.
- Зміни клієнтів (додавання, видалення, зміна секрету, увімкнення/вимкнення, ad-tag) застосовуються **«на льоту»** через management-API `mtg`, не розриваючи Telegram-сесії решти користувачів; лише структурні зміни inbound (адреса, domain fronting, ліміт з'єднань, публічні IP) перезапускають процес.
...
| Домен прикриття (FakeTLS) | `settings.fakeTlsDomain` | За замовчуванням `www.cloudflare.com`. З 3.5.0 — **домен за замовчуванням для генерації секретів нових клієнтів** (підказка: «Default FakeTLS domain used to generate a new client's secret. Each client can front its own domain»); кожен клієнт може прикриватися власним доменом через свій секрет. |
| Max connections | `settings.throttleMaxConnections` | **Нове в 3.5.0.** Обмеження одночасних з'єднань на всіх користувачів із чесним розподілом; `0` — без обмеження. |
| Public IPv4 / Public IPv6 | `settings.publicIpv4` / `settings.publicIpv6` | **Нове в 3.5.0.** Публічна адреса сервера для ad-tag middle proxy (плейсхолдери `1.2.3.4` / `2001:db8::1`); порожньо — `mtg` визначить сам. |

**Формат секрету (FakeTLS).** Секрет тепер живе **у кожного клієнта** (поле **«MTProto secret»** у формі клієнта, з кнопкою перегенерації; поруч — необов'язковий **«Ad-tag (спонсорський канал)»**, рівно 32 hex-символи). Вигляд секрету попередній: `ee` + 32 hex-символи + hex-код домену прикриття (`ee<hex32><hex(домен)>`); при прив'язці нового клієнта до MTProto-inbound секрет генерується автоматично від домену inbound за замовчуванням. Квота трафіку і термін дії клієнта з 3.5.0 **реально застосовуються** (через ліміти mtg-multi): вичерпаний або прострочений клієнт відключається, а скидання трафіку повертає доступ одразу.
...
(еквівалент — `https://t.me/proxy?server=…&port=…&secret=…`). З 3.5.0 посилання **персональне** — будується із секрету конкретного клієнта, а фрагмент `#remark` з нього прибрано (нестрогі парсери Telegram склеювали його із секретом і ламали імпорт); примітка показується окремою міткою у вікні інформації. Це посилання та QR-код потрібно надіслати користувачу Telegram — при відкритті проксі одразу додається до застосунку. Посилання також повертається через сервер підписок.
...
| Max Connections | `maxConnections` | `6` | Максимум фізичних з'єднань (`0` — без обмеження). З версії 3.4.2 нові inbound створюються зі значенням `6`; раніше створені зберігають своє. |
...
| **Ціль** (`target`) | `""` (з 3.4.2 при увімкненні REALITY залишається порожнім) | **Обов'язкове поле.** Реальний домен, чиє TLS-рукостискання позичає REALITY. Дослівна підказка: «*Обов'язково. Має містити порт (наприклад, example.com:443). Без порту Xray-core не запускається.*» Валідація в панелі перевіряє наявність і коректність порту; інакше виводяться помилки «Ціль REALITY обов'язкова» / «Ціль REALITY має містити порт…» / «У цілі REALITY вказано неприпустимий порт». Поруч із полем — кнопки **«Сканувати»** (перевірити поточну ціль «наживо») і **«Знайти цілі»** (відкрити сканер цілей REALITY); див. нижче. |
| **SNI** (`serverNames`) | `[]` (підставляється разом із ціллю) | Список допустимих SNI (множинне введення тегами). Має відповідати домену з **Ціль**. При успішному скануванні цілі SNI заповнюється за її сертифікатом. |
...
| **Мін. версія клієнта** (`minClientVer`) | `""` | Мінімальна версія Xray-клієнта (плейсхолдер з 3.5.0 — `26.3.27`). Порожньо — без обмеження. |
...
З версії 3.4.2 **Ціль** (`target`) і **SNI** (`serverNames`) при увімкненні REALITY більше **не** заповнюються автоматично — обидва поля залишаються порожніми, а ціль підбирається живим сканером (див. нижче). Обирайте «важкий», стабільний сторонній HTTPS-сайт з підтримкою TLS 1.3 і HTTP/2, що не знаходиться за вашим же сервером.

#### Пошук цілі REALITY (живий сканер)

З версії 3.4.2 попередня кнопка «випадкова ціль» (зі статичним вбудованим списком) замінена двома діями поруч із полем **Ціль**:

- **«Сканувати»** — «наживо» перевіряє поточну ціль: встановлює TLS-з'єднання і, якщо ціль придатна, підставляє SNI з її сертифіката. Тости: «Ціль придатна — заповнено ціль і SNI.» / «Ціль доступна, але не придатна для REALITY.» / «Не вдалося просканувати ціль REALITY.».
- **«Знайти цілі»** — відкриває вікно **«Сканер цілей REALITY»**: можна вказати домен, **IP або діапазон CIDR** (тоді панель знаходить цілі за сертифікатами живих хостів) або залишити поле порожнім (перевіряються вбудовані кандидати). Результати ранжуються; стовпці — «Статус»/«Придатно», «Обмін ключами», «Сертифікат», `TLS`, `ALPN`, «Затримка». Кнопкою **«Використати»** вибраний рядок підставляється в поля inbound.

Ціль вважається **придатною**, якщо сервер узгоджує **TLS 1.3**, **HTTP/2 (ALPN h2)**, обмін ключами **X25519/X25519MLKEM768** і пред'являє **довірений** (не wildcard) сертифікат. Сканування виконується на стороні сервера панелі (`POST /panel/api/server/scanRealityTarget` і `…/scanRealityTargets`).
...
- **Інформація про клієнта** — картка з усіма полями, використаним/залишковим трафіком (**Залишок**), терміном дії та прив'язаними inbound. З 3.5.0 картка також показує **групу** клієнта (рядок «Група» з міткою, над «Коментар»), коли групу задано.
...
- **Змінити ({count})** / **Коригування** (`POST /bulkAdjust`) — масова зміна терміну та/або квоти. Діалог «Змінити {count} клієнтів» з підказкою «Позитивні значення додають, від'ємні — зменшують. Клієнти з необмеженим терміном або трафіком пропускаються для відповідного поля.». Поля: **Додати дні**, **Додати трафік (ГБ)** і **Set flow**. Логіка:
  ...
  - **Авто-увімкнення (з 3.4.2):** клієнт, вимкнений **лише через вичерпання** (минув термін або перевищено квоту), при продовженні автоматично знову вмикається — локально і на його вузлі, — якщо коригування повертає його в межі лімітів. Вимкнені вручну або все ще вичерпані залишаються вимкненими (поле `enable` тепер записується явно, тому стан зберігається і у клієнтів без inbound).
...
- **Прив'язати ({count})** / **Відв'язати ({count})** (`POST /bulkAttach` / `bulkDetach`) — масове прив'язування/відв'язування вибраних клієнтів до вибраних inbound. Цілі — тільки багатокористувацькі inbound. Результат відв'язування: «Відв'язано {detached}, пропущено {skipped}.». З 3.5.0 селектор inbound у формі клієнта **приховує вимкнені inbound** (крім тих, до яких клієнт уже прив'язаний), а у вікні «Відв'язати» усунено накопичення дублів записів одного клієнта (дані виправляються автоматично при старті панелі).
...
Над списком є рядок пошуку («Пошук email, коментаря, sub ID, UUID, пароля, auth, Telegram ID…») — він шукає за email, коментарем, subId, UUID, паролем, auth і (знову з 3.5.0) за **Telegram ID**. Лічильник результатів: «Показано {shown} з {total}».
...
З 3.5.0 у таблиці є стовпець **«Швидкість»** (між «Трафік» і «Залишок»): жива швидкість клієнта — синя мітка `↑ віддача / ↓ завантаження` (ковзне ~5-секундне середнє, оновлюється кожні 5 секунд); за відсутності трафіку — сіра риска. На мобільних швидкість показується рядком у картці клієнта.
...
Поведінка: обнуляється **лише відображуваний лічильник самої групи** — панель запам'ятовує поточну суму `up/down` групи як базову відмітку і далі в списку груп показує `max(0, сума − базова відмітка)`. Лічильники `up`/`down`, квоти і поле `enable` окремих клієнтів **не змінюються**, перезапуск Xray не потрібен. Це зміна поведінки відносно попередніх версій, де скидання обнуляло трафік усіх клієнтів групи.

Запит: `POST /panel/api/clients/groups/resetTraffic` з тілом `{"name": "<ім'я групи>"}`.

Повідомлення про успіх: **«Трафик группы {name} сброшен.»**
...
З таблиці груп, як і раніше, доступні дії над групою в цілому, зокрема **«Сбросить трафик»** — обнуляє **лише лічильник самої групи** (базову відмітку), не чіпаючи трафік окремих клієнтів (див. [9.9](#99-скидання-трафіку-групи)). Після такого скидання колонка «Использованный трафик» для цієї групи показує `0`.
...
| Поле (UI) | Ключ | За замовчуванням | Опис |
|---|---|---|---|
| Інтервали оновлення підписки | `subUpdates` | `12` | Як часто (у годинах) клієнтський застосунок повинен перезапитувати підписку. Підказка: «Інтервал між оновленнями в клієнтському застосунку (у годинах)». З 3.5.0 поле приймає **0–525600** і показує діапазон (попередня межа інтерфейсу 168 не збігалася із сервером і після апгрейду з 2.x блокувала збереження будь-яких налаштувань). |
...
- **Ідентифікація клієнта:** `{{EMAIL}}` (синонім — `{{USERNAME}}`), `{{INBOUND}}` (ремарка самого inbound), `{{HOST}}` (ремарка хоста), `{{ID}}` (UUID), `{{SHORT_ID}}` (перші 8 символів UUID), `{{SUB_ID}}`, `{{COMMENT}}`, `{{TELEGRAM_ID}}`, `{{PROTOCOL}}`, `{{TRANSPORT}}`.
...
З версії 3.4.2 змінна `{{EMAIL}}` (і синонім `{{USERNAME}}`) виводиться лише на **першому** посиланні клієнта в тілі підписки — як і блок залишку трафіку/терміну; на решті посилань того самого клієнта вона опускається.
...
| Оголошення | `subAnnounce` | `Announce` (у Base64) | «Текст оголошення, що відображається у VPN-клієнті». З 3.5.0 текст також показується **інформаційним банером угорі інфо-сторінки підписки** (коли непорожній), а кастомним шаблонам доступна змінна `announce`. |
...
**З версії 3.5.0 хост — це «група»**: один запис охоплює **кілька inbound** і **кілька адрес** одразу. Дії списку (увімкнення/вимкнення/видалення/перестановка) та API працюють із групами (рядковий `groupId`); з'явився масовий ендпоінт `POST /panel/api/hosts/bulk/add` (`{"inboundIds": [...], "hosts": [...], ...}`), а `list`/`get`/`update`/`del`/`setEnable` оперують об'єктами групи.

У кожного хоста (групи) задаються:

- **Remark** і опис (Description), прив'язка до **Inbounds** (мульти-вибір з пошуком; мінімум один), перемикач **Enable** і призначення на вузли (**Nodes**).
- **Address** — з 3.5.0 це **список адрес** (введення тегами; розділювачі — кома, крапка з комою, пробіл; плейсхолдер `cdn.example.com, cdn2.example.com:443`). У кожного запису може бути свій вбудований `:порт` (зокрема IPv6 у дужках `[::1]:443`); випадаюча підказка пропонує адреси, вже використані іншими хостами. Порожній список — успадковується адреса самого inbound (у списку це помаранчева мітка **Inherits**). **Port** (`0` — успадковується порт inbound) служить портом за замовчуванням для записів без вбудованого порту; **Tags** (враховуються лише в RAW-підписці).
...
- Вкладка **Advanced** — Host header, Path, маршрут VLESS, Mux, Sockopt, Final Mask і виключення хоста з окремих форматів підписки (raw / json / clash). З 3.5.0 **Final Mask хоста потрапляє і в raw-посилання** (`fm=`; раніше — лише в JSON/Clash): TCP/UDP-маски хоста додаються до масок inbound, QUIC-параметри хоста беруться, лише якщо їх немає в inbound. Перемикач **Allow insecure** тепер діє і для **Hysteria/Hysteria2** (`insecure=1` у посиланні, `skip-cert-verify: true` у Clash).
- Вкладка **Clash (mihomo)** — версія IP, Mihomo X25519, перемішування хостів (Shuffle host).

У списку стовпець **Endpoint** показує адреси чіпами (перший видно, решта — у поповері «+N»), а стовпець **Inbounds** — чіпи inbound, розфарбовані за протоколом. З 3.5.0 хости впорядковуються **глобально** (за порядком сортування, потім за приміткою), а не в межах inbound; масові вмикання, вимикання та видалення зберігаються. Керовані хости замінюють попередній масив External Proxy.

**Маршрут VLESS (VLESS route).** З версії 3.4.2 це одне число `0-65535` (а не список портів; підказка — «одне значення VLESS route (0-65535), що зашивається в UUID, наприклад 443; порожньо — без нього», плейсхолдер `443`). Задане значення реально «зашивається» в UUID кожної згенерованої підписки (raw / JSON / Clash): Xray читає байти 6-7 UUID і маскує їх перед автентифікацією, тому клієнт як і раніше збігається. Порожнє або некоректне значення UUID не змінює.
...
Повертає повну JSON-конфігурацію (формат, зрозумілий sing-box і похідним клієнтам — Podkop, OpenWRT sing-box, Karing, NekoBox). З 3.5.0 у JSON-підписку потрапляють і клієнти **нативних WireGuard-inbound** (`secretKey`, `address`, `peers[]` з `publicKey`/`endpoint`/`preSharedKey`/`keepAlive`/`allowedIPs`, `mtu`) — раніше вони мовчки пропускалися. Для цього формату доступні додаткові параметри (вкладка `subFormats`):
...
Формат генерованих посилань і YAML підтримується в актуальному стані для сучасних клієнтів: Shadowsocks-2022 (SS2022) більше не кодує userinfo в Base64; посилання Shadowsocks з http-обфускацією видаються у форматі SIP002 з плагіном `obfs-local`; для підписок Clash/Mihomo реалізовано повний набір полів XHTTP. З 3.5.0 у Clash/Mihomo-підписку потрапляють і клієнти **нативних WireGuard-inbound** (поля `private-key`, `public-key`, `pre-shared-key`, `persistent-keepalive`, `ip`/`ipv6`, `mtu`, `dns`) — раніше вони мовчки пропускалися. Окремих налаштувань це не потребує — посилання просто коректніше розпізнаються клієнтами.
...
Якщо відкрити **будь-яке з трьох посилань підписки** — raw, JSON або Clash — у браузері (або явно додати до URL параметр `?html=1` або `?view=html`, або надіслати заголовок `Accept: text/html`), сервер замість «сирої» відповіді видає візуальну **сторінку інформації про підписку** («Інформація про підписку»). До 3.5.0 так працювало лише основне посилання `/sub/`, а `/json/` і `/clash/` віддавали в браузер сирий JSON/YAML. VPN-клієнти як і раніше отримують машинну відповідь, оскільки вони не запитують HTML.
...
Окремий Happy Eyeballs є і в **налаштуваннях dial (sockopt) конкретного outbound**: з 3.5.0 він справді вмикається (раніше конфіг серіалізувався як вимкнений і перемикач «злітав»). «Try delay (ms)» за замовчуванням тепер **250**; явно заданий `0` (= вимкнено) зберігається; також доступні Prioritize IPv6, interleave (1) і maxConcurrentTry (4).
...
#### Default Outbound (з 3.5.0)

Перший пункт вкладки **«Базова маршрутизація»** — селектор **Default Outbound**: «Трафік, що не збігся з жодним правилом маршрутизації, використовує цей outbound (Xray бере перший outbound у списку)». У списку — `direct`, `blocked` і всі наявні теги; за замовчуванням `direct`. Вибір **переміщує вказаний outbound на першу позицію** шаблону; відсутні `direct`/`blocked` створюються на льоту, а при переключенні не видаляються.
...
| Мережа | — | `network` | `tcp`, `udp` або `tcp,udp`. З 3.4.2 в конфіг записується малими літерами (`tcp`/`udp`), а в таблиці маршрутів відображається великими (`TCP`/`UDP`); порожньо — будь-яка мережа. |
...
| Target Strategy | **Target Strategy** | **Нове в 3.5.0.** Як резолвиться домен призначення перед підключенням. 11 значень: `AsIs` (за замовчуванням, не резолвити), `UseIP`, `UseIPv4`, `UseIPv6`, `UseIPv6v4`, `UseIPv4v6` (резолв із fallback), `ForceIP`, `ForceIPv6v4`, `ForceIPv6`, `ForceIPv4v6`, `ForceIPv4` (вимагають успішного резолву). Порожньо = `AsIs` (у конфіг не пишеться); для `freedom` значення читається з fallback на попередній `domainStrategy` (легасі-ключ як і раніше записується для старих ядер). |
...
З 3.5.0 додано новий тип **TCP-маски** — **XMC (Minecraft)** (`xmc`): потік маскується під протокол Minecraft. Поля: **Hostname** (необов'язково; «адреса сервера, що імітується в рукостисканні»), **Usernames** (необов'язковий список; «імена гравців, що пропонуються проберам; за замовчуванням ядро використовує Dream») і обов'язковий **Password** (16 символів, автогенерація з кнопкою ↻; «пароль обфускації»). Важливо: **поєднання TCP-Finalmask з REALITY відхиляється при збереженні** — воно призводило до падіння Xray-core при першому підключенні (помилка: «Finalmask is not supported with REALITY security…»; див. Xray-core#6453). UDP-маска з REALITY допустима; збережені раніше некоректні комбінації автоматично «лікуються» (маска відкидається) при збиранні конфігу.
...
- **Real delay** (**нове в 3.5.0**) — третій режим перемикача. Використовує той самий тимчасовий екземпляр Xray, але повідомляє **повний час «холодного» запиту, включно зі встановленням тунелю** — цифру, близьку до тієї, що показують клієнтські застосунки. Підказка перемикача: «TCP: швидкий dial-пробник. HTTP: повний запит через xray. Real delay: повний час, включно зі встановленням з'єднання». Звичайний **HTTP**-режим з 3.5.0 вимірюється за «теплим» (keep-alive) з'єднанням, тому його цифри нижчі й ближчі до затримки одного запиту.
...
#### Стовпці Egress і Country (з 3.5.0)

Після успішного **HTTP** або **Real delay** тесту панель через той самий тимчасовий маршрут запитує метадані Cloudflare trace і показує у списку outbound два стовпці: **Egress** — IP виходу (IPv4/IPv6, кожен прихований за «оком»; поки тесту не було — риска з підказкою «Виконайте HTTP-тест, щоб показати IP виходу і країну») і **Country** — прапор і назва країни виходу; якщо вихід іде через Cloudflare WARP, додається помаранчева мітка WARP. TCP-тест ці стовпці не заповнює; на мобільних дані показуються в картці outbound.
...
| Fallback | **Fallback** | Резервний тег, якщо жоден селектор не підійшов. **З 3.5.0 можна вибрати й інший балансувальник** (у списку такі варіанти позначені синьою міткою «Balancer», підказка «Виберіть інший балансувальник як fallback»). Xray нативно цього не вміє, тому панель сама будує прихований loopback-outbound `_bl_<ціль>` і правило маршрутизації (шлях: балансувальник → loopback → сервер → цільовий балансувальник → outbound; попередження про зайвий «хоп» показується у формі). Є захист від циклів (помилка «не можна вибрати — виникла б циклічна залежність», варіант у списку блокується з підказкою шляху циклу) і від видалення використовуваного балансувальника («Не можна видалити — використовується як fallback у: …»). Префікс тегу `_bl_` зарезервований (у тегах заборонений); службові `_bl_`-об'єкти приховані зі списків Outbounds/Routing, а стовпці Fallback/Live Target/Override показують ім'я цільового балансувальника, а не loopback-тег. |
...
**З версії 3.4.2** сторінку «Балансувальники» розділено на вкладки **«Налаштування балансувальника»** (таблиця і додавання) та **«Observatory»**, а спостерігач редагується **структурною формою** (попередній сирий JSON-редактор прибрано). Панель сама створює і видаляє спостерігачів за потреби: звичайний `observatory` — для стратегії **Least Ping**, розширений `burstObservatory` — для **Least Load**, а також для **Random/Round-robin із заданим `fallbackTag`** (цього вимагає xray-core; при очищенні останнього `fallbackTag` спостерігач прибирається). Створення або видалення спостерігача вимагає повного перезапуску Xray.
...
| `pingConfig.httpMethod` | **Нове в 3.4.2.** HTTP-метод проб: `HEAD` (за замовчуванням) або `GET`. |

На вкладці **«Observatory»** ті самі параметри задаються формою (а не JSON). Для звичайного `observatory` це **Watched Outbounds** (`subjectSelector`, лише читання), **Probe URL** (`probeURL`, за замовчуванням `https://www.google.com/generate_204`), **Probe Interval** (`probeInterval`, `1m`) і **Enable Concurrency** (`enableConcurrency`, за замовчуванням увімкнено). Для `burstObservatory` — перелічені вище поля плюс нове **«HTTP-метод»**. Якщо у балансувальника є обидва спостерігачі, між формами Observatory і Burst перемикає сегментний перемикач.

Як все пов'язати:

1. На вкладці **«Observatory»** налаштуйте параметри спостерігача (він створюється автоматично під вибрану стратегію; `subjectSelector` слідує за селекторами балансувальника).
...
#### Що відбувається при видаленні outbound або балансувальника (з 3.4.2)

При видаленні outbound або балансувальника панель у тій самій дії **вичищає посилання на нього в маршрутизації** і показує **попередній перегляд наслідків** у діалозі підтвердження (заголовок «При видаленні також оновиться ваша маршрутизація:», далі по кожному правилу — «видалено (не залишилося призначення)» або «залишено (тепер використовує …)», і «Балансувальник … видалено (не залишилося цілей)»). Так запобігається «висяче» посилання (`balancerTag`/`outboundTag`/`dialerProxy`), через яке раніше xray-core міг не запуститися при наступному перезапуску. Видалення балансувальника: правила зберігають свій `outboundTag`, інакше видаляються. Видалення outbound: його тег прибирається із селекторів і `fallbackTag` балансувальників, очищається `dialerProxy` в інших outbound, спустілі балансувальники видаляються разом із тепер-безпредметними правилами, спостерігачі перезбираються.

### 11.6. DNS

Розділ `dns`. Увімкнення: **Увімкнути DNS** (підказка: *«Увімкнути вбудований DNS-сервер»*).

> З 3.5.0 DNS-сервери на **приватних IP** (наприклад, свій AdGuard/Pi-hole у тій самій docker-мережі) працюють «з коробки»: панель автоматично вставляє і підтримує кероване allow-правило (direct, обмежене портом DNS-сервера) **перед** блокуючим правилом `geoip:private` і синхронізує його при зміні `dns.servers`. Раніше власні DNS-запити Xray мовчки блокувалися цим правилом, додаючи ~4 с затримки на кожен новий домен; ручні правила більше не потрібні (створені вручну не зачіпаються).
...
### 12.9. Виправлення 3.5.0 (стабільність вузлів)

Пакет виправлень у 3.5.0, помітних оператору вузлів:

- **Збереження клієнта без змін більше не руйнує трафік вузла.** Раніше будь-яке збереження клієнта (навіть «відкрив — зберіг») надсилало на вузол повне оновлення inbound з повторним створенням обробника Xray — вузол значився онлайн, але не пропускав трафік до ручного перезапуску. Тепер no-op збереження нічого не роблять, а реальні правки йдуть одним легким оновленням без повторного створення.
- **Host-перевизначення вузла приймаються в майстер**: при першому прийнятті inbound вузла його рядки Hosts (SNI, відбиток, ALPN тощо) імпортуються, і підписки несуть правильні TLS-параметри.
- **Авто-продовження на вузлі відкриває свіже вікно квоти** (пакет виду «100 ГБ кожні 30 днів» знову видає повний обсяг).
- **Видалення клієнта на майстері повністю видаляє його на вузлах** (запис + трафік), а не лише відв'язує; залишки попередніх масових видалень можна прибрати дією «Видалити неприв'язаних клієнтів» на вузлі.
- **Inbound'и вузла не вимітаються до першого прийняття**: збереження щойно доданого вузла більше не може видалити його живі inbound'и.
- **Один некоректний inbound не зупиняє синхронізацію трафіку вузла** (заразом легасі-inbound'и `socks` на вузлах перейменовуються в `mixed`).
- **Перевірка конфлікту портів обмежена своїм вузлом** — правка адреси прослуховування вузлового inbound більше не відхиляється через такий самий порт на іншому вузлі («port … already used by inbound …»).

### 12.10. Вузли: нове в 3.3.0
...
З версії 3.5.0 у меню «/» Telegram зареєстровано **вісім** команд:
...
| `/usage` | Показати витрати клієнта: /usage email | всі (див. нижче) | Додана в меню в 3.5.0. |
| `/inbound` | Пошук inbound: /inbound remark (admin) | адмін | Додана в меню в 3.5.0. |
| `/restart` | Перезапустити ядро Xray (admin) | адмін | Додана в меню в 3.5.0. |
| `/clearall` | Скинути трафік усіх клієнтів (admin) | адмін | **Нова в 3.5.0**: запитує підтвердження (кнопки «Скасувати» / «Підтвердити скидання трафіку») і обнуляє трафік усіх клієнтів. |

Деталі команд-аргументів:
...
Ще три зміни 3.5.0: у списку **онлайн-клієнтів** кнопки підписані `email - примітка inbound` (однакові імена в різних inbound більше не плутаються); повідомлення **бекапу** і **бан-лога** починаються з рядка `Hostname==…` — зручно, коли один бот обслуговує кілька панелей; пошук картки клієнта за **Telegram ID** працює незалежно від форматування JSON налаштувань (раніше «компактні» записи, що прийшли з вузлів або з імпорту, не знаходилися).
...
- **PostgreSQL** — якщо панель налаштована на PostgreSQL, у блоці відображається підказка:
  > «Ця панель працює на PostgreSQL. „Резервна копія" завантажує архів pg_dump (.dump), а „Відновлення" завантажує його назад через pg_restore. Відновлення також приймає базу SQLite (.db) або міграційний дамп SQLite та імпортує їхні дані в PostgreSQL. На сервері мають бути встановлені клієнтські інструменти PostgreSQL (pg_dump і pg_restore).»
...
|-----------|-----------|----------------------------|
| SQLite | `{host}_РРРР-ММ-ДД_ГГХВСС.db` | Спочатку виконується checkpoint WAL, щоб файл містив останні записи, потім файл цілком читається і передається для завантаження |
| PostgreSQL | `{host}_РРРР-ММ-ДД_ГГХВСС.dump` | Запускається `pg_dump`, архів передається для завантаження |

З версії 3.4.2 ім'я завантажуваного файлу резервної копії містить адресу, за якою ви відкрили панель, і **дату-час**: `{host}_РРРР-ММ-ДД_ГГХВСС` з розширенням `.db` (SQLite) або `.dump` (PostgreSQL) — наприклад `panel.example.com_2026-06-27_000000.db`. Так файли групуються за сервером і сортуються за часом, а кілька копій за один день не перезаписують одна одну. Те саме ім'я використовується в резервних копіях, що надсилає бот Telegram (`host` береться з домену/IP панелі, запасний варіант — `x-ui`).
...
Підказки в інтерфейсі:
- SQLite: «Натисніть, щоб вибрати і завантажити файл .db або міграційний дамп (.dump) з вашого пристрою для відновлення бази даних.»
- PostgreSQL: «Натисніть, щоб вибрати і завантажити бекап PostgreSQL (.dump), базу SQLite (.db) або міграційний дамп SQLite для відновлення бази даних. Це замінить усі поточні дані.»

З 3.5.0 тип файлу визначається за вмістом (`PGDMP` — архів pg_dump; заголовок «SQLite format 3» — база `.db`; початкові `PRAGMA`/`BEGIN TRANSACTION` — текстовий SQL-дамп), а не за розширенням; діалог вибору приймає `.dump,.db` на обох СУБД. SQLite-файли, завантажені на PostgreSQL-панель, імпортуються в PostgreSQL однією транзакцією (див. 3.14); перед імпортом файл перевіряється як справжня панельна БД, старі схеми автоматично домігруються. Перед відновленням `pg_dump`-архіву панель заздалегідь (до зупинки Xray) перевіряє його читабельність: якщо дамп створено новішим PostgreSQL, виводиться помилка з точною командою — наприклад, «…run 'x-ui pgclient 17'…».
...
З 3.5.0 кнопка показується **лише на PostgreSQL-панелях** (на SQLite звичайний бекап `.db` тепер відновлюється прямо в PostgreSQL — окремий експорт не потрібен):
...
Підказка: «Натисніть, щоб завантажити базу даних SQLite (.db), зібрану з ваших даних PostgreSQL і готову для запуску панелі на SQLite.»

Перетворення `.db ⇄ .dump` для SQLite можна виконати і з CLI командою `x-ui migrateDB [file]` (див. розділ 16.7). Також з 3.5.0 міграція між рушіями стала **транзакційною і без втрат** (переносяться зокрема групи клієнтів і глобальні лічильники трафіку; невдалий імпорт не змінює цільову БД), а для оновлення клієнтських інструментів PostgreSQL з'явилася команда **`x-ui pgclient [версія]`** і пункт **10. Install/Upgrade client tools (pg_dump/pg_restore)** у меню PostgreSQL (за потреби підключається офіційний репозиторій PostgreSQL).
...
**Dev-канал (rolling), з 3.4.2.** Кнопка версії панелі на «Дашборді» тепер завжди відкриває вікно оновлення (раніше на актуальній стабільній збірці вона вела на сторінку релізів GitHub), а перемикач **«Dev-канал»** (`devChannelEnable`) показується завжди — навіть на стабільній збірці. Увімкнувши його, при наступному оновленні можна перейти на «котючий» dev-канал (збірки за кожним комітом у `main`). При увімкненні виводиться попередження: «Dev-збірки відстежують кожен коміт у main і не є стабільними релізами — автоматичного відкату немає». Без явної дії користувача нічого не оновлюється.
...
#### Виправлення скриптів у 3.5.0

- **RHEL-сімейство** (Rocky/Alma/RHEL/Oracle): локальне встановлення PostgreSQL з меню тепер працює (автентифікація за паролем замість ident), а fail2ban для ліміту IP ставиться з **EPEL**.
- **Arch/Manjaro**: встановлення та оновлення більше не запускають повний апгрейд системи `pacman -Syu` — лише оновлення бази пакетів.
- **32-бітний ARM**: завантажений бінарник Xray іменується `xray-linux-arm32` — саме так, як його запускає панель (раніше оновлення клало файл під іншим ім'ям, і панель продовжувала ганяти старе ядро).
- **IP-сертифікат**: автовизначений публічний IPv4 показується для підтвердження (Enter — прийняти) перед випуском; при відмові — перевірене ручне введення.
- **Вибір ACME-порту**: Enter (прийняти порт 80 за замовчуванням) більше не виводить хибне «Your input is invalid».
...
| **Очищення логів** | **щодня** (`@daily`) | Очищає IP-limit логи і persistent access-log, ротуючи поточний лог у `*.prev.log`. З 3.5.0 щоденне очищення зачіпає **і error-лог Xray** (раніше — лише access) |
| **Обмеження росту логів Xray** | **кожні 10 хвилин** (`@every 10m`) | **Нове в 3.5.0.** Обрізає access- і error-лог Xray, коли будь-який з них перевищує **64 МіБ**; вимкнені логи (`none`/порожньо) не чіпаються |
