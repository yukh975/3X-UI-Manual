...
**Версия 3X-UI: 3.8.5.** Руководство составлено по этой версии и актуально для неё. Сводка изменений 3.8.5 относительно 3.8.0 — в разделе [«Что нового в 3.8.5»](#что-нового-в-385).
...
- [Что нового в 3.8.5](#что-нового-в-385)
...
#### Обновление на 3.8.5: что учесть

- **Встроенная страница профиля подписки теперь выключена по умолчанию** (`subProfileMode` = `none`): заголовок `Profile-Web-Page-Url` больше не отправляется установкам, которые никогда не задавали свой URL профиля. Если ваши пользователи пользуются этой страницей, откройте «Настройки → Подписка» и переключите режим страницы профиля на **Built-in**. Ранее заданный свой URL сохраняется и переносится в режим **Custom** — действий не требуется (см. [10.2](#102-настройки-сервера-подписок)).
- **«Restart Xray After Client Disable» теперь срабатывает и при ручном отключении или удалении клиента**, а не только при авто-отключении по сроку или квоте. Если перезапуск ядра при ручных правках клиента не нужен, выключите эту настройку на вкладке «Панель» в настройках (см. [13.7](#137-внешний-трафик-и-поведение-xray-вкладка-внешний-трафик--external-traffic)).
- **Сохранение или включение inbound'а с конфликтом портов теперь отклоняется**, а не роняет ядро молча. Если после обновления сохранение или включение отклонено — освободите порт, названный в сообщении об отказе (там указан inbound-владелец или пир AmneziaWG, который его занимает).
- **Разовая миграция `DNSOutboundQTypeZeroFix`** переписывает сохранённый в DNS-outbound числовой `"qType": 0` в строку `"0"`. Установки, дошедшие до 3.8.0 с правилом «блокировать тип 0» в DNS-outbound, теряли через него **весь** DNS; миграция чинит шаблон на месте. Непарсящийся шаблон логируется и пропускается — сбой не фатален, панель стартует.
- **Узлу передаются только IP его клиентов.** Мастер больше не рассылает всей ферме полную таблицу IP на каждом 10-секундном тике — действий не требуется.
- **Требования прежние:** ядро Xray-core v26.9.9; сборка из исходников — Go 1.27.1. Установка готовыми пакетами и скриптом этого не касается.
- При переходе с версии младше 3.8.0 дополнительно выполняются миграции 3.8.0 (см. [«Что нового в 3.8.0»](https://github.com/yukh975/3X-UI-Manual/blob/3.8.0/3X-UI-MANUAL.ru.md#что-нового-в-380) в руководстве к версии 3.8.0).
...
С 3.8.5 та же проверка конфликта порта выполняется не только при сохранении, но и при **включении** уже существующего inbound (переключатель «Enable»): раньше отключённый inbound мог держать порт незамеченным, и конфликт всплывал только при попытке его включить. Теперь включение отклоняется с той же ошибкой, что и при сохранении, — она называет строку-владельца порта, — а флаг `enable` не переключается. Отдельно (и при сохранении, и при включении) отклоняется inbound на порту, который уже форвардит пир **AmneziaWG** (клиентская переадресация портов), — ошибка называет email этого клиента.
...
**Клик по карточке сводки (3.8.5).** Карточки над списком (Клиенты, В сети, Исчерпано, Почти исчерпано, Отключено, Активна) кликабельны: клик по карточке ставит единственным фильтром её статусную категорию, повторный клик снимает фильтр, а клик по карточке «Клиенты» сбрасывает фильтр по статусу. Заодно выровнены сами категории «Активна»/«Отключено» — раньше они были шире, чем счётчик на карточке (в «Активна» попадали клиенты на грани исчерпания, в «Отключено» — уже исчерпанные отключённые), теперь фильтр и карточка всегда показывают одно и то же число.
...
| Страница профиля | `subProfileMode` | управляет `Profile-Web-Page-Url` | Режим ссылки на встроенную страницу профиля: **Без ссылки** (`none`, по умолчанию) — заголовок не отправляется; **Встроенная** (`builtin`) — ссылка на встроенную страницу текущего запроса подписки (см. [10.5](#105-страница-информации-о-подписке-и-qr-коды)); **Свой сайт** (`custom`) — адрес из поля «URL профиля» ниже. |
| URL профиля | `subProfileUrl` | `Profile-Web-Page-Url` | «Ссылка на ваш сайт, отображаемая в VPN-клиенте». Показывается и применяется только в режиме «Свой сайт»; пустое значение заголовок не отправляет. |
...
**Обновление до 3.8.5.** Раньше при пустом `subProfileUrl` в заголовок `Profile-Web-Page-Url` автоматически подставлялся фактический URL запроса подписки — ссылка на встроенную страницу профиля уходила всегда, даже без явной настройки, и раскрывала сам адрес подписки. С 3.8.5 при отсутствии сохранённого `subProfileMode` пустой `subProfileUrl` переводится в режим «Без ссылки», а непустой прежний адрес — в режим «Свой сайт» без повторного ввода. Чтобы вернуть прежнее поведение, выберите «Встроенная»; учтите, что встроенная страница раскрывает адреса подписки и конфигураций.
...
С 3.8.5 страница подписки перестроена: вверху — кольцевой индикатор использования с процентом (или «∞» для безлимитных подписок) и остатком либо использованным трафиком, рядом — сетка краткой статистики (дней до окончания, срок действия, статус, загружено/отправлено/лимит, последний онлайн). Остальное содержимое разбито на вкладки **«Подписка»**, **«Приложения»** и **«Конфиги»**. В подвале вкладки «Подписка» — ссылка техподдержки (`subSupportUrl`, если задана) и интервал автообновления клиента (`subUpdates`, см. [10.2](#102-настройки-сервера-подписок)). Для языков `fa-IR` и `ar-EG` страница отображается справа налево; адреса, идентификаторы и объёмы данных остаются слева направо.
...
- **Информацию о подписке** (блок Descriptions):
  ...
  - «Статус» — «Активна», «Неограниченно», «Истёк», «Трафик исчерпан» или «Неактивна» (для выключенного клиента). С 3.8.5 три причины неактивности (истёк срок, исчерпан трафик, клиент выключен) различаются, а не сводятся к одному значению «Неактивна»;
...
- **Кнопки быстрого импорта в приложения** (вкладка «Приложения», переключатель платформы Android/iOS — по умолчанию платформа посетителя по `User-Agent`): у каждого приложения одна кнопка «Добавить». Для Android она открывает deep-link v2box, v2rayNG (`v2rayng://install-config?url=…`), Sing-box (`sing-box://import-remote-profile?url=…`), V2RayTun (`v2raytun://import/…`), Happ (`happ://add/…`) или Incy (`incy://add/…`); для iOS — Shadowrocket (параметр `flag=shadowrocket`), v2box (`v2box://install-sub?url=…&name=…`), Streisand (`streisand://import/…`), V2RayTun, Happ или Incy. С 3.8.5 Sing-box и V2RayTun на Android и V2RayTun на iOS тоже открывают приложение (раньше только копировали ссылку), а NPV Tunnel убран из списка обеих платформ.
...
**3.8.5:** сохранённый `qType: 0` у правила DNS-outbound ядро читает как «тип не задан», поэтому такое правило совпадало с любым DNS-запросом через outbound — блокируя, отклоняя или хайджекая весь DNS вместо одного типа запроса. Исправлено: `0` теперь пишется как строка `"0"`. Одноразовая миграция `DNSOutboundQTypeZeroFix` чинит уже сохранённые шаблоны — актуально, если апгрейд до 3.8.0 превратил старый `blockTypes: [0]` в такой `qType: 0`; шаблон с нечитаемым JSON миграция пропускает, лишь пишет предупреждение в лог, не прерывая запуск панели.
...
#### Перезапускать Xray после отключения клиента (*Restart Xray After Client Disable*)

- **Ключ:** `restartXrayOnClientDisable`
- **Значение по умолчанию:** `true`
- **Назначение:** Xray перезапускается, чтобы разорвать уже установленные соединения клиента, который перестал обслуживаться, — **автоматически** (по истечении срока действия или лимита трафика) **или вручную** (отключение или удаление клиента, в т.ч. массовое). Подсказка: «Когда клиент перестаёт обслуживаться — автоматически (по сроку или лимиту трафика) или вручную — перезапускать Xray, чтобы его активные соединения оборвались.» Переключатель находится на вкладке «Панель» (*General*) рядом с прочими общими настройками.
- **Изменение в 3.8.5.** Раньше (*Restart Xray After Auto Disable*) настройка срабатывала только при автоотключении: снятие клиента через API ядра (`RemoveUser`) удаляет лишь учётные данные и не разрывает уже установленную сессию (VLESS, VMess, Trojan, Shadowsocks её сохраняют), поэтому вручную отключённый или удалённый клиент мог продолжать передавать трафик. Теперь ручное отключение/удаление клиента (в форме клиента и при массовом отключении) при включённой настройке тоже вызывает перезапуск Xray. Действует только для локальных inbound'ов панели, выполняющей операцию, — правка клиента на узле не заставляет мастер перезапускать свой собственный Xray.
...
| Перезапускать Xray после отключения клиента (*Restart Xray After Client Disable*) | `restartXrayOnClientDisable` | `true` | Перезапускать Xray, когда клиент перестаёт обслуживаться — автоматически (по истечении срока или лимита трафика) или вручную (отключение/удаление, в т.ч. массовое). С 3.8.5 распространяется и на ручное отключение/удаление (раньше — только автоотключение). Подсказка: «Когда клиент перестаёт обслуживаться — автоматически (по сроку или лимиту трафика) или вручную — перезапускать Xray, чтобы его активные соединения оборвались.» **Переключатель находится на вкладке «Панель» (*General*)** — см. п. 13.2; здесь приведён для полноты. |

**Отказ в конфигурации, которую ядро не может забиндить (с 3.8.5).** Если сгенерированная конфигурация назначает двум inbound'ам один и тот же адрес и порт с пересекающимся транспортом (TCP+TCP или UDP+UDP; сочетание разных транспортов на одном порту, например TCP и UDP, — штатно и конфликтом не считается), панель отклоняет такую конфигурацию на единой точке перезапуска Xray, **не** останавливая уже работающее ядро. Раньше промах бинда убивал сам процесс Xray, а секундный цикл перезапуска-наблюдателя раз за разом повторял ту же ошибку, укладывая разом все протоколы. Причина отказа (вида «config refused: …») отображается на панели рядом со статусом Xray, пока ядро продолжает работать на прежней (последней успешно применённой) конфигурации. Коллизия, которую уже обслуживает работающее ядро, не считается ошибкой — отклоняются только новые пересечения адреса и порта.
