...
**Версия 3X-UI: 3.7.0.** Руководство составлено по этой версии и актуально для неё. Сводка изменений 3.7.0 относительно 3.6.0 — в разделе [«Что нового в 3.7.0»](#что-нового-в-370).
...
- [Что нового в 3.7.0](#что-нового-в-370)
...
- [5. Протоколы](#5-протоколы)
  ...
  - [5.13. Краткая шпаргалка по выбору протокола](#513-краткая-шпаргалка-по-выбору-протокола)
...
#### Обновление на 3.7.0: что учесть

- **Сделайте бэкап базы перед обновлением.** При первом старте 3.7.0 выполняются миграции схемы: колонки цикла сброса трафика и времени последней выгрузки подписки у клиента, служебные колонки синхронизации узлов, индекс по `LOWER(email)`, область действия и срок годности у API-токенов, нормализация внешних ссылок.
- **Сборка из исходников требует Go 1.27** (в 3.6.0 было 1.26.5). Установка готовыми пакетами и скриптом этого не касается. Версия ядра Xray в 3.7.0 не менялась.
- Ваши собственные файлы в `bin/` (например, вручную положенный `geoip.dat`) переживают обновление, а `update.sh` скачивает файлы через прокси, настроенный у панели.
...
Здесь `v25.6.8` — тег из списка, который отдаёт `GET /getXrayVersion`. Версия обязана присутствовать в этом списке, иначе панель ответит отказом. С версии 3.4.2 минимально допустимая для установки версия Xray поднята до **26.6.27**, а в комплекте 3.5.0 идёт ядро **Xray 26.7.11**. Попутные изменения ядра: шифры Shadowsocks `none`/`plain` и VMess `none`/`zero` удалены (сохранённые конфиги переписываются автоматически: SS — на поддерживаемый шифр, VMess — на `auto`), а незашифрованный VLESS/Trojan-outbound на публичный адрес отклоняется при сохранении — ядро с таким конфигом не запустилось бы.
...
> Начиная с версии **3.3.0** в перечень добавлено значение `mtproto` (прокси Telegram), с **3.7.0** — `amneziawg` (WireGuard с обфускацией против DPI).
...
Для протоколов `http`, `mixed`, `tunnel`, `tun`, `wireguard`, `amneziawg` вкладка транспорта недоступна — у них нет stream-настроек Xray. У `amneziawg`, как и у `mtproto`, недоступен и блок sniffing: соединения обслуживает не Xray.
...
| «Общий ключ WireGuard» (PSK) | Необязательный дополнительный общий ключ |
...
| Поле | Значение по умолчанию | Описание |
|---|---|---|
| `privateKey` / `publicKey` | генерируются | Ключевая пара сервера; публичный ключ выводится из приватного |
...
| `H1`–`H4` | «Магические заголовки» — число или диапазон `низ-верх`; пусто = классические 1/2/3/4 |
| `I1`–`I5` | Необязательные сигнатурные пакеты; пусто — не отправляются |
| `HeaderProtectionKey` | Base64-ключ 32 байта для защиты заголовков; должен совпадать на всех клиентах. Пусто — защита выключена. **Если ключ задан, все `S1`–`S4` обязаны быть ≥ 12** — панель проверяет это при сохранении, а не только ядро при запуске |
...
**Параметры клиента** — во вкладке учётных данных клиента (показываются, когда привязанный inbound — AmneziaWG): **AmneziaWG Private Key**, **Public Key**, **Pre-Shared Key**, **Allowed IPs** (пусто — адрес назначается автоматически; записи разделяются запятыми) и **Forwarded Ports** — порты и диапазоны, пробрасываемые (DNAT) на этого клиента, например `80, 443, 8000-8100`; пусто — проброса нет.
...
### 5.13. Краткая шпаргалка по выбору протокола
...
| **Мин. версия клиента** (`minClientVer`) | `""` | Минимальная версия Xray-клиента; формат и диапазон проверяются при сохранении (плейсхолдер — `x.y.z`). С 3.6.0 «пусто» **не** означает «без ограничения»: ядро применяет встроенный минимум версии клиента (`26.3.27`). |
| **Макс. версия клиента** (`maxClientVer`) | `""` | Максимальная версия Xray-клиента; формат/диапазон проверяются при сохранении. Пусто — верхнего предела нет. |
...
- **«Сканировать»** — «вживую» проверяет текущую цель: устанавливает TLS-соединение и, если цель пригодна, подставляет SNI из её сертификата. Тосты: «Цель пригодна — заполнены цель и SNI.» / «Цель доступна, но не пригодна для REALITY.» / «Не удалось просканировать цель REALITY.».
- **«Найти цели»** — открывает окно **«Сканер целей REALITY»**: можно указать домен, **IP или диапазон CIDR** (тогда панель находит цели по сертификатам живых хостов) либо оставить поле пустым (проверяются встроенные кандидаты). Результаты ранжируются; столбцы — «Статус»/«Пригодно», «Обмен ключами», «Сертификат», `TLS`, `ALPN`, «Задержка». Кнопкой **«Использовать»** выбранная строка подставляется в поля inbound.

Цель считается **пригодной**, если сервер согласует **TLS 1.3**, **HTTP/2 (ALPN h2)**, обмен ключами **X25519/X25519MLKEM768** и предъявляет **доверенный** (не wildcard) сертификат. Сканирование выполняется на стороне сервера панели (`POST /panel/api/server/scanRealityTarget` и `…/scanRealityTargets`).
...
**Лимит устройств (HWID).** Поле **«HWID Limit»** (`limitHwid`) — максимум устройств, с которых принимаются запросы подписки; `0` — без ограничения. Рядом список **«HWID Devices»**: устройство, ОС и её версия, модель, время первого и последнего обращения. Устройство удаляется по одному (`DELETE /panel/api/clients/hwids/:email/:id`) или списком целиком (`DELETE /panel/api/clients/hwids/:email`) — удалённое просто зарегистрируется заново при следующей выгрузке подписки, так что это способ «отвязать телефон», а не наказание.

#### Внешние ссылки клиента: что добавилось в 3.7.0

Сама вкладка **«Links»** (сторонние share-ссылки и внешние подписки в выдаче клиента) появилась раньше и разобрана в разделе [10.1](#101-что-такое-subid-и-как-формируется-ссылка). В 3.7.0 у каждой строки появилось управление: **переключатель `enable`** — временно выключить строку, не удаляя её; **срок действия** — после него строка выпадает из выдачи; **префикс имени**; **время последней выгрузки** и **текст последней ошибки**, если внешний источник не ответил. Значение по-прежнему проверяется при сохранении: share-ссылка должна разбираться, внешняя подписка — быть `http(s)`.
...
С версии 3.4.2 переменная `{{EMAIL}}` (и синоним `{{USERNAME}}`) выводится только на **первой** ссылке клиента в теле подписки — как и блок остатка трафика/срока; на остальных ссылках того же клиента она опускается.
...
Раздел настроек **«Sub Balancers»** добавляет в **JSON-подписку** отдельные профили, которые сами выбирают лучший из ваших серверов. Каждый включённый балансировщик — это один дополнительный профиль в выдаче; в конфиге клиента он разворачивается в `routing.balancers` + `burstObservatory`, то есть выбор делает клиентское приложение по своим замерам, а не панель.

| Поле | Описание |
|---|---|
| **Remark** | Название профиля в списке клиента (плейсхолдер — `Auto · fastest`) |
| **Strategy** | Как выбирается сервер: **Least load** (наименьшая нагрузка), **Least ping** (наименьший пинг), **Random**, **Round robin** |
| **Inbounds** | Какие inbound'ы участвуют — их endpoint'ы и станут кандидатами |
| **Order** | Позиция в списке подписки; нумерация общая с порядком inbound'ов, при равных номерах балансировщик идёт **после** inbound'а |
| **Enabled** | Выключенный балансировщик в подписку не попадает |

Управление — через `GET/POST /panel/api/sub-balancers`, `POST /panel/api/sub-balancers/:id` (изменение) и `DELETE /panel/api/sub-balancers/:id` (есть POST-алиас `/del` для клиентов, не умеющих DELETE).
...
- **Remark** и описание (Description), привязка к **Inbounds** (мульти-выбор с поиском; минимум один), переключатель **Enable** и назначение на узлы (**Nodes**).
...
В списке колонка **Endpoint** показывает адреса чипами (первый виден, остальные — в поповере «+N»), а колонка **Inbounds** — чипы inbound, раскрашенные по протоколу. С 3.5.0 хосты упорядочиваются **глобально** (по порядку сортировки, затем по примечанию), а не в пределах inbound; массовые включение/выключение/удаление сохраняются. Управляемые хосты приходят на смену прежнему массиву External Proxy.
...
Возвращает полную JSON-конфигурацию (формат, понятный sing-box и производным клиентам — Podkop, OpenWRT sing-box, Karing, NekoBox). С 3.5.0 в JSON-подписку попадают и клиенты **нативных WireGuard-inbound** (`secretKey`, `address`, `peers[]` с `publicKey`/`endpoint`/`preSharedKey`/`keepAlive`/`allowedIPs`, `mtu`) — раньше они молча пропускались. Для этого формата доступны дополнительные параметры (вкладка `subFormats`):
...
- **`x-ui setting -getApiToken` больше не плодит токены.** Раньше каждый вызов создавал новый админ-токен, и в панели копились «забытые» ключи с полным доступом. Теперь команда **ротирует единственный токен `cli-fallback`**: выданные ранее CLI-токены после ротации перестают работать — если вы использовали их в скриптах, обновите значение.
