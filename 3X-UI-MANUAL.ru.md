...
🇬🇧 [Версия на английском](3X-UI-MANUAL.md)

**Версия 3X-UI: 3.4.0.** Руководство составлено по этой версии и актуально для неё. Сводки изменений: [«Что нового в 3.4.0»](#что-нового-в-340) (относительно 3.3.1) и [«Что нового в 3.3.1»](#что-нового-в-331) (относительно 3.3.0).
...
Этот раздел кратко перечисляет изменения версии **3.4.0** относительно 3.3.1, которые видны пользователю панели, сгруппированные по разделам руководства. Подробности по каждой функции — в соответствующем разделе ниже. (Изменения предыдущей версии — в разделе [«Что нового в 3.3.1»](#что-нового-в-331).)
...
- **История системы (графики).** Кнопка/раздел «Графики» → «История системы» с вкладками: «Пропускная способность», «Пакеты», «Диск I/O», «Онлайн», «Нагрузка», «Соединения», «Использование диска». Данные тянутся по `GET /history/:metric/:bucket`; допустимые интервалы агрегации (bucket, сек): **2, 30, 60, 120, 180, 300, 720, 1440, 2880** (последние три соответствуют пресетам **12h**, **24h** и **48h** в селекторе интервала), на вкладку приходит до 60 точек. Кольцевой буфер метрик хранит данные за период до **48 часов**, поэтому графики (CPU, RAM, трафик, пакеты, соединения, диск, онлайн, нагрузка) можно просматривать за период до двух суток. Допустимые метрики: `cpu, mem, swap, netUp, netDown, pktUp, pktDown, diskRead, diskWrite, diskUsage, tcpCount, udpCount, online, load1, load5, load15`. Подпись «Последние 2 минуты» соответствует bucket = 2 (режим реального времени).
...
**Пример.** Получить ряд загрузки ЦП за последние ~2 минуты (bucket = 2 с, до 60 точек) и тот же ряд, агрегированный по 5 минут (bucket = 300 с):
  ...
  Метрику можно заменить на любую допустимую (`mem`, `netUp`, `tcpCount`, `load1` и т. д.). Bucket вне списка `2, 30, 60, 120, 180, 300, 720, 1440, 2880` будет отвергнут.
- **Метрики Xray** — отдельный блок с потреблением памяти и сборкой мусора Xray (ряды `xrAlloc, xrSys, xrHeapObjects, xrNumGC, xrPauseNs`) и «Обсерваторией» (состояние исходящих соединений). Работают, только если в конфигурации Xray задан блок `metrics` (`listen 127.0.0.1:11111`, тег `metrics_out`); иначе показывается «Конечная точка метрик Xray не настроена».
...
- **Аутентификация X25519** (англ. «X25519 auth») — генерирует пару `decryption`/`encryption` на базе X25519.
- **Аутентификация ML-KEM-768** (англ. «ML-KEM-768 auth») — пост-квантовый вариант (Post-Quantum).
- **Очистить** — сбрасывает оба поля обратно в `none`.

Под кнопками выводится строка состояния «Выбрано: {auth}», где значение определяется по последнему сегменту строки `encryption`: пусто/`none` → «None», очень длинный ключ (>300 символов) → ML-KEM-768, короткий → X25519, иначе «Свой».
...
- **Изменить ({count})** / **Корректировка** (`POST /bulkAdjust`) — массовое изменение срока и/или квоты. Диалог «Изменить {count} клиентов» с подсказкой «Положительные значения добавляют, отрицательные — уменьшают. Клиенты с неограниченным сроком или трафиком пропускаются для соответствующего поля.». Поля: **Добавить дни** и **Добавить трафик (ГБ)**. Логика:
  ...
  - Если ни дни, ни трафик не указаны: «Укажите дни или трафик перед применением.». Тост: «Изменено: {count}» / «Изменено: {ok}, пропущено: {skipped}».
...
- **Лог** (см. [11.9](#119-логи-и-статистика-stats-metrics)).
...
Служебное правило статистики (`inboundTag: ["api"] → outboundTag: "api"`) отключить нельзя — его переключатель заблокирован, чтобы не сломать учёт трафика панели (см. [11.10](#1110-сохранение-перезапуск-и-автоматические-преобразования)).
...
- **`socks`**, **`http`** — список `settings.servers[]` с `address`/`port`; поле **Пароль авторизации**.
...
> **Изменено в 3.3.1.** Отдельная настройка «Сетевой прокси панели» (`panelProxy`) убрана. Исходящий трафик самой панели (в том числе запросы к WARP API) теперь направляется через выбранный **outbound для трафика панели** — Xray-outbound или балансировщик (см. [«Что нового в 3.3.1»](#что-нового-в-331) и раздел [13](#13-настройки-панели)). Описание ниже относится к версиям до 3.3.1.
...
Все параметры бота задаются в панели в разделе **Настройки → Telegram-бот**. После изменения настроек их нужно сохранить и перезапустить панель — бот инициализируется при старте веб-сервера.
