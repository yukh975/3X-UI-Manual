...
Nhấp lại công tắc sẽ mở cửa sổ «Отключить двухфакторную аутентификацию» với gợi ý «Введите код из приложения, чтобы отключить двухфакторную аутентификацию.». Sau khi nhập mã đúng (từ 3.4.2 mã được kiểm tra **ở phía máy chủ**), cờ và secret được xóa, hiển thị «Двухфакторная аутентификация была успешно удалена».
...
**Xác nhận bằng mã 2FA (từ 3.4.2).** Nếu bảng điều khiển đã bật xác thực hai yếu tố, việc thay đổi tên đăng nhập/mật khẩu còn yêu cầu thêm việc **nhập mã hiện tại** từ ứng dụng xác thực. Cửa sổ **«Thay đổi thông tin đăng nhập»** mở ra với gợi ý «Введите код из приложения, чтобы изменить учётные данные администратора.»; mã (`twoFactorCode`) được kiểm tra ở phía máy chủ, khi mã sai hoặc trống thì thay đổi không được áp dụng. Cùng một xác nhận đó nay cũng được yêu cầu khi tắt 2FA (xem [2.2](#22-xác-thực-hai-yếu-tố-2fa--totp)).
...
| Sử dụng TLS (LDAPS) | «Использовать TLS (LDAPS)» (`useTls`) | **false** | Khi bật, sử dụng scheme `ldaps://` (mặc định — với kiểm tra chứng chỉ máy chủ). |
| Bỏ qua kiểm tra chứng chỉ TLS | «Bỏ qua kiểm tra chứng chỉ TLS» (`ldapInsecureSkipVerify`) | **false** | Thêm vào 3.4.2. Tắt việc kiểm tra chứng chỉ máy chủ khi dùng LDAPS — dành cho CA nội bộ/tự ký. Gợi ý: «Không an toàn — tắt việc kiểm tra chứng chỉ máy chủ. Chỉ dùng với các CA nội bộ/không tin cậy». Công tắc **chỉ** khả dụng khi đã bật «Sử dụng TLS (LDAPS)». |
...
Trong menu bên của bảng điều khiển (và trong ngăn kéo trên di động), kể từ phiên bản 3.4.2 có nút **«Tài liệu»** (biểu tượng quyển sách) — mở tài liệu chính thức `https://docs.sanaei.dev/`. Ngoài ra, trong 3.4.2 đã thực hiện cải tiến **khả năng truy cập** xuyên suốt: các nút biểu tượng và các phần tử có thể nhấp đã nhận được nhãn aria và khả năng kích hoạt bằng Enter/Space (cho trình đọc màn hình và điều hướng bằng bàn phím).
...
- **«Копировать ссылку»** (tiếng Anh: *Copy URL*), **«Экспорт ссылок»**. Kể từ phiên bản 3.4.2, thao tác toàn trang **«Xuất tất cả liên kết»** (`GET /panel/api/inbounds/allLinks`; cửa sổ «Xuất tất cả liên kết inbound», tệp «All-Inbounds») tạo liên kết qua engine đăng ký — áp dụng mẫu nhận xét cho từng khách hàng và ưu tiên các endpoint Host được quản lý (trước đây sử dụng nhận xét cố định `inbound-email`).
...
Mục đích: inbound WireGuard — tunnel VPN WireGuard, chứ không phải proxy có giả mạo. Transport và TLS/REALITY không áp dụng được cho nó.

**Kể từ phiên bản 3.4.2, WireGuard chuyển sang mô hình đa khách hàng** (như VLESS/VMess/Trojan/Shadowsocks/Hysteria). Bản thân inbound chỉ lưu phần máy chủ; các thiết bị peer nay được thêm dưới dạng **khách hàng** thông thường (xem phần 8) — với việc tự động cấp địa chỉ trong tunnel, hỗ trợ đăng ký, giới hạn lưu lượng/thời hạn và nhóm. Danh sách inline «Peer / Thêm peer» cũ trong biểu mẫu inbound đã bị xóa: inbound mới được tạo không có peer, còn mỗi khách hàng được tự động cấp địa chỉ.

Các trường inbound (khối `settings`):

| Trường | Giá trị mặc định | Mô tả |
|---|---|---|
| `secretKey` | — | Khóa riêng của máy chủ (bắt buộc). Có nút tạo bên cạnh; khóa công khai (`Public Key`) được suy ra từ nó tự động (chỉ đọc; `pubKey` lưu riêng trước đây đã bị xóa) |
| `mtu` | `1420` | MTU của giao diện |
| `dns` | `1.1.1.1, 1.0.0.1` | **Mới trong 3.4.2.** DNS được ghi vào dòng `DNS =` của `.conf` khách hàng |
| `noKernelTun` | `false` (tắt) | «TUN không kernel» — userspace-TUN thay vì kernel |
| `domainStrategy` | (tùy chọn) | «Domain Strategy»: `ForceIP`, `ForceIPv4`, `ForceIPv4v6`, `ForceIPv6`, `ForceIPv6v4` |

**Tham số WireGuard của khách hàng** — tab **«Thông tin đăng nhập»** trong cửa sổ thêm/sửa khách hàng (hiển thị khi inbound được liên kết là WireGuard):
...
Ngoài những trường đã liệt kê, inbound WireGuard còn có trường **Domain Strategy** (chiến lược phân giải domain: `ForceIP`, `ForceIPv4`, `ForceIPv4v6`, `ForceIPv6`, `ForceIPv6v4`). Trường không bắt buộc và chỉ ghi vào cấu hình khi được thiết lập.
...
Với khách hàng WireGuard, trong các cửa sổ «Thông tin»/QR có **thẻ cấu hình có thể thu gọn**: `.conf` đầy đủ (`[Interface]` với `PrivateKey`/`Address`/`DNS`/`MTU` và `[Peer]` với `PublicKey`/`PresharedKey`/`AllowedIPs = 0.0.0.0/0, ::/0`/`Endpoint`/`PersistentKeepalive`) cùng các nút **Sao chép**, **Tải xuống** và **QR**. Cũng hỗ trợ liên kết dạng `wireguard://`/`wg://`, được ứng dụng client phân tích thành `.conf` sẵn sàng.
...
| Max Connections | `maxConnections` | `6` | Số kết nối vật lý tối đa (`0` — không giới hạn). Kể từ phiên bản 3.4.2, các inbound mới được tạo với giá trị `6`; các inbound được tạo trước đó giữ nguyên giá trị của mình. |
...
Kể từ phiên bản 3.4.2, **Đích** (`target`) và **SNI** (`serverNames`) khi bật REALITY **không** còn được điền tự động — cả hai trường vẫn để trống, còn đích được chọn bằng trình quét trực tiếp (xem bên dưới). Hãy chọn một trang HTTPS bên thứ ba ổn định, «nặng cân», có hỗ trợ TLS 1.3 và HTTP/2, không nằm trên cùng máy chủ của bạn.

#### Tìm đích REALITY (trình quét trực tiếp)

Kể từ phiên bản 3.4.2, nút «đích ngẫu nhiên» cũ (với danh sách tích hợp tĩnh) được thay bằng hai thao tác bên cạnh trường **Đích**:

- **«Quét»** — kiểm tra «trực tiếp» đích hiện tại: thiết lập kết nối TLS và, nếu đích phù hợp, điền SNI theo chứng chỉ của nó. Thông báo: «Đích phù hợp — đã điền đích và SNI.» / «Đích khả dụng, nhưng không phù hợp cho REALITY.» / «Không quét được đích REALITY.».
- **«Tìm đích»** — mở cửa sổ **«Trình quét đích REALITY»**: có thể chỉ định tên miền, **IP hoặc dải CIDR** (khi đó panel tìm các đích theo chứng chỉ của các host đang hoạt động) hoặc để trống trường (kiểm tra các ứng viên tích hợp). Kết quả được xếp hạng; các cột — «Trạng thái»/«Phù hợp», «Trao đổi khóa», «Chứng chỉ», `TLS`, `ALPN`, «Độ trễ». Bằng nút **«Sử dụng»**, dòng đã chọn được điền vào các trường inbound.

Đích được coi là **phù hợp** nếu máy chủ thương lượng **TLS 1.3**, **HTTP/2 (ALPN h2)**, trao đổi khóa **X25519/X25519MLKEM768** và xuất trình chứng chỉ **được tin cậy** (không phải wildcard). Việc quét được thực hiện ở phía máy chủ của panel (`POST /panel/api/server/scanRealityTarget` và `…/scanRealityTargets`).
...
- **Sửa ({count})** / **Điều chỉnh** (`POST /bulkAdjust`) — thay đổi hàng loạt thời hạn và/hoặc hạn mức. Hộp thoại «Sửa {count} client» với chú thích «Giá trị dương thêm vào, giá trị âm giảm đi. Các client với thời hạn hoặc lưu lượng không giới hạn bị bỏ qua cho trường tương ứng.». Các trường: **Thêm ngày**, **Thêm lưu lượng (GB)** và **Set flow**. Logic:
  ...
  - **Tự động bật (từ 3.4.2):** client đã bị tắt **chỉ vì hết hạn mức** (hết hạn hoặc vượt quá hạn mức), khi gia hạn sẽ tự động được bật lại — cục bộ và trên node của nó — nếu việc điều chỉnh đưa nó trở lại trong giới hạn. Những client bị tắt thủ công hoặc vẫn còn hết hạn mức vẫn bị tắt (trường `enable` nay được ghi rõ ràng, nên trạng thái được giữ cả với các client không có inbound).
...
- Nội dung: **«Chỉ đặt lại bộ đếm lưu lượng của nhóm. Bộ đếm của từng khách hàng không bị ảnh hưởng.»**

Hành vi: chỉ **bộ đếm hiển thị của chính nhóm** được đặt về 0 — bảng điều khiển ghi nhớ tổng `up/down` hiện tại của nhóm làm mốc cơ sở và sau đó trong danh sách nhóm hiển thị `max(0, tổng − mốc cơ sở)`. Các bộ đếm `up`/`down`, hạn mức và trường `enable` của từng khách hàng **không bị thay đổi**, không cần khởi động lại Xray. Đây là thay đổi hành vi so với các phiên bản trước, nơi việc đặt lại xóa lưu lượng của tất cả khách hàng trong nhóm.

Yêu cầu: `POST /panel/api/clients/groups/resetTraffic` với body `{"name": "<tên nhóm>"}`.

Thông báo thành công: **«Đã đặt lại lưu lượng của nhóm {name}.»**
...
Từ bảng nhóm, các thao tác đối với toàn bộ nhóm vẫn khả dụng, bao gồm **«Đặt lại lưu lượng»** — chỉ đặt về 0 **bộ đếm của chính nhóm** (mốc cơ sở), không động đến lưu lượng của từng khách hàng (xem [9.9](#99-đặt-lại-lưu-lượng-của-nhóm)). Sau khi đặt lại như vậy, cột «Lưu lượng đã sử dụng» cho nhóm đó hiển thị `0`.
...
- **Nhận dạng client:** `{{EMAIL}}` (đồng nghĩa — `{{USERNAME}}`), `{{INBOUND}}` (remark của chính inbound), `{{HOST}}` (remark của host), `{{ID}}` (UUID), `{{SHORT_ID}}` (8 ký tự đầu của UUID), `{{SUB_ID}}`, `{{COMMENT}}`, `{{TELEGRAM_ID}}`, `{{PROTOCOL}}`, `{{TRANSPORT}}`.
...
Kể từ phiên bản 3.4.2, biến `{{EMAIL}}` (và đồng nghĩa `{{USERNAME}}`) chỉ hiển thị trên liên kết **đầu tiên** của client trong nội dung đăng ký — giống như khối lưu lượng/thời hạn còn lại; trên các liên kết khác của cùng client nó bị bỏ qua.
...
| Mạng | — | `network` | `tcp`, `udp` hoặc `tcp,udp`. Từ 3.4.2 được ghi vào cấu hình bằng chữ thường (`tcp`/`udp`), còn trong bảng định tuyến hiển thị bằng chữ hoa (`TCP`/`UDP`); để trống — mọi mạng. |
...
**Kể từ phiên bản 3.4.2** trang «Bộ cân bằng tải» được chia thành các tab **«Cài đặt bộ cân bằng tải»** (bảng và thêm mới) và **«Observatory»**, còn bộ quan sát được chỉnh sửa bằng **biểu mẫu có cấu trúc** (trình chỉnh sửa JSON thô cũ đã bị xóa). Bảng điều khiển tự tạo và xóa các bộ quan sát khi cần: `observatory` thông thường — cho chiến lược **Least Ping**, `burstObservatory` mở rộng — cho **Least Load**, cũng như cho **Random/Round-robin với `fallbackTag` đã đặt** (điều này do xray-core yêu cầu; khi xóa `fallbackTag` cuối cùng, bộ quan sát bị loại bỏ). Việc tạo hoặc xóa một bộ quan sát yêu cầu khởi động lại hoàn toàn Xray.
...
| `pingConfig.httpMethod` | **Mới trong 3.4.2.** Phương thức HTTP của các thăm dò: `HEAD` (mặc định) hoặc `GET`. |

Trên tab **«Observatory»** các tham số tương tự được đặt bằng biểu mẫu (chứ không phải JSON). Đối với `observatory` thông thường, đó là **Watched Outbounds** (`subjectSelector`, chỉ đọc), **Probe URL** (`probeURL`, mặc định `https://www.google.com/generate_204`), **Probe Interval** (`probeInterval`, `1m`) và **Enable Concurrency** (`enableConcurrency`, mặc định bật). Đối với `burstObservatory` — các trường đã liệt kê ở trên cộng thêm trường mới **«Phương thức HTTP»**. Nếu bộ cân bằng tải có cả hai bộ quan sát, một công tắc phân đoạn sẽ chuyển giữa các biểu mẫu Observatory và Burst.

Cách kết nối tất cả:

1. Trên tab **«Observatory»** cấu hình các tham số của bộ quan sát (nó được tạo tự động theo chiến lược đã chọn; `subjectSelector` đi theo các bộ chọn của bộ cân bằng tải).
...
#### Điều gì xảy ra khi xóa outbound hoặc bộ cân bằng tải (từ 3.4.2)

Khi xóa outbound hoặc bộ cân bằng tải, trong cùng thao tác bảng điều khiển **dọn sạch các tham chiếu đến nó trong định tuyến** và hiển thị **xem trước hậu quả** trong hộp thoại xác nhận (tiêu đề «Khi xóa, định tuyến của bạn cũng sẽ được cập nhật:», sau đó với mỗi quy tắc — «đã xóa (không còn đích)» hoặc «giữ lại (nay dùng …)», và «Bộ cân bằng tải … đã xóa (không còn mục tiêu)»). Nhờ đó tránh được tham chiếu «treo» (`balancerTag`/`outboundTag`/`dialerProxy`) mà trước đây có thể khiến xray-core không khởi động được ở lần khởi động lại tiếp theo. Xóa bộ cân bằng tải: các quy tắc giữ lại `outboundTag` của chúng, nếu không thì bị xóa. Xóa outbound: thẻ của nó được loại khỏi các bộ chọn và `fallbackTag` của các bộ cân bằng tải, `dialerProxy` của các outbound khác được xóa, các bộ cân bằng tải trống được xóa cùng với các quy tắc nay không còn đối tượng, các bộ quan sát được dựng lại.
...
|-----------|-----------|----------------------------|
| SQLite | `{host}_YYYY-MM-DD_HHMMSS.db` | Đầu tiên thực hiện checkpoint WAL để tệp chứa các bản ghi mới nhất, sau đó toàn bộ tệp được đọc và gửi để tải xuống |
| PostgreSQL | `{host}_YYYY-MM-DD_HHMMSS.dump` | Chạy `pg_dump`, tệp lưu trữ được gửi để tải xuống |

Kể từ phiên bản 3.4.2, tên tệp sao lưu được tải xuống chứa địa chỉ mà bạn đã mở bảng điều khiển, và **ngày-giờ**: `{host}_YYYY-MM-DD_HHMMSS` với phần mở rộng `.db` (SQLite) hoặc `.dump` (PostgreSQL) — ví dụ `panel.example.com_2026-06-27_000000.db`. Nhờ đó các tệp được nhóm theo máy chủ và sắp xếp theo thời gian, còn nhiều bản sao trong cùng một ngày không ghi đè lên nhau. Cùng tên đó được dùng trong các bản sao lưu mà bot Telegram gửi (`host` được lấy từ miền/IP của bảng điều khiển, phương án dự phòng — `x-ui`).
...
**Kênh dev (rolling), từ 3.4.2.** Nút phiên bản bảng điều khiển trên «Bảng điều khiển chính» nay luôn mở cửa sổ cập nhật (trước đây trên bản dựng ổn định mới nhất, nó dẫn đến trang phát hành GitHub), còn công tắc **«Kênh dev»** (`devChannelEnable`) luôn hiển thị — kể cả trên bản dựng ổn định. Khi bật nó, ở lần cập nhật tiếp theo có thể chuyển sang kênh dev «rolling» (các bản dựng theo mỗi commit vào `main`). Khi bật, một cảnh báo được hiển thị: «Các bản dựng dev theo dõi mỗi commit vào main và không phải là bản phát hành ổn định — không có rollback tự động». Nếu không có thao tác rõ ràng của người dùng, sẽ không có gì được cập nhật.
